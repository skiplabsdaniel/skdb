...
- db.database: The name of the current executed database (if --init or --data supplied).
...
- db.operation: The type of the SQL Statemement.
...
By default the monitoring trace is written to stderr, a SK_MONITORING_WRITER environment variable can be defined with following value:

- 'fifo' to write the trace into '/tmp/skmonitor.trace' fifo.
- 'file' to write the trace into 'skmonitor.log' file.
- 'fifo:<name>' to write the trace into '<name>' fifo.
- 'file:<name>' to write the trace into '<name>' file.
...
It's creates a '/tmp/skmonitor.trace' fifo and read it.
...
- condition: An boolean expression the condition the trace
  - In that expression the following varaible can be used
    - name: The name of the span
    - value: The current value of the metrics
    - status: The status of the span (UNSET | OK | ERROR)
    - duration_(s|ms|µs|ns): The duration if the span
