...
void NewClass(
  ...
  if (args.IsConstructCall()) {
    ...
    args.This()->DefineProperty(context, SKFrozen(isolate), pd).FromJust();
...
Local<Symbol> SKFrozen(Isolate* isolate) {
  return Symbol::For(isolate, FromUtf8(isolate, "SkipRuntime.frozen"));
}

Local<Symbol> SKIsArrayProxy(Isolate* isolate) {
  return Symbol::For(isolate, FromUtf8(isolate, "SkipRuntime.isArrayProxy"));
}

Local<Symbol> SKIsObjectProxy(Isolate* isolate) {
  return Symbol::For(isolate, FromUtf8(isolate, "SkipRuntime.isObjectProxy"));
}
