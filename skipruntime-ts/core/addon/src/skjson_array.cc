...
Local<Array> AsArray(Isolate* isolate, void* json) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    jsArr->Set(context, i, SKStoreToNode(isolate, field, true)).FromJust();
  }
  return jsArr;
}
...
Local<Value> ArrayGet(Isolate* isolate, Local<Object> target, Local<Value> prop,
  ...
  String::Utf8Value utf8Prop(isolate, prop);
  ...
  Local<External> external = target->Get(context, FromUtf8(isolate, "hdl"))
                                 ...
                                 .As<External>();
  if (prop->IsSymbol()) {
    if (strProp == "Symbol(sk_isArrayProxy)")
      return Boolean::New(isolate, true);
    if (strProp == "Symbol(sk_frozen)") return Boolean::New(isolate, true);
    if (strProp == "Symbol(Symbol.iterator)") {
      return AsArray(isolate, external->Value())
          ->Get(context, Symbol::GetIterator(isolate))
          .ToLocalChecked();
    }
  ...
  if (strProp == "toString") {
    return skbinding::JSONStringify(isolate, handler);
  }
  if (strProp == "forEach") {
    return AsArray(isolate, external->Value())
        ->Get(context, FromUtf8(isolate, "forEach"))
        .ToLocalChecked();
  }
  try {
...
Local<Value> ArrayHas(Isolate* isolate, Local<Object> target,
  ...
  if (strProp == "toString") return Boolean::New(isolate, true);
  if (strProp == "forEach") return Boolean::New(isolate, true);
