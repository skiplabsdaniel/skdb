...
CJCustom SKIP_SKJSON_createCJCustom(PartialCJObj obj, char* type);
}
...
void CreateCJCustom(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  if (args.Length() != 2) {
    // Throw an Error that is passed back to JavaScript
    isolate->ThrowException(
  ...
  NatTryCatch(isolate, [&args](Isolate* isolate) {
    SKIP_SKJSON_createCJCustom(args[0].As<External>()->Value(),
                               ToSKString(isolate, args[1].As<String>()));
  });
}
...
void GetBinding(const FunctionCallbackInfo<Value>& args) {
  ...
  AddFunction(isolate, binding, "SKIP_SKJSON_createCJCustom", CreateCJCustom);
