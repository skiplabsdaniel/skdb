...
// prettier-ignore
import type { ptr, float, Opt, Environment, Links, ToWasmManager, Utils, Shared } from "#std/sk_types.js";
import type {
  Attribute,
  AttributeValue,
  ScalarValue,
...
function convertAttributes(skAttributes: SkAttributes): Attribute[] {
  ...
  for (const key in keys) {
    attributes.push({ key: key, value: convertValue(skAttributes[key]) });
  }
  return attributes;
}
...
class LinksImpl implements Links {
  ...
  complete = (utils: Utils, exports: object) => {
    this.write = (skContent: ptr) => {
      if (!this.skmonitor.monitor) return;
      this.collect(utils.importString(skContent));
    };
    this.traceIdOpt = () => {
      const traceId = this.skmonitor.traceId();
      return traceId ? utils.exportString(traceId) : null;
    };
    this.monitoringOpt = () => {
      const mode = this.skmonitor.monitorMode();
      return mode ? utils.exportString(mode) : null;
    };
    this.environment.shared.set("SKMonitor", this.skmonitor);
  };

  private async collect(strSkSpan: string) {
