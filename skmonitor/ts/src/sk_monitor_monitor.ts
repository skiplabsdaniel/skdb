...
export class Monitor {
  ...
  constructor(maxSize?: number) {
    this.tracers = [];
    this.meters = [];
    this.maxSize = maxSize !== undefined ? maxSize : 4000;
    this.traceSize = 0;
    this.metricSize = 0;
    this.traceExporters = [];
  ...
  createTracers(resource: Attribute[]): Tracers {
    ...
    const tracers = new Tracers(resource, this.notifyTrace);
    this.tracers.push(tracers);
    return tracers;
  }

  createMeters(resource: Attribute[]): Meters {
    const meters = new Meters(resource, this.notifyMetrics);
...
export function create(config: Config): Monitor {
  ...
  config.uptraceUrls.forEach((uptrace) => {
    const trace = uptrace.trace === undefined ? true : uptrace.trace;
    const metrics = uptrace.metrics === undefined ? true : uptrace.metrics;
    if (trace) {
  ...
  config.optlUrls.forEach((optl) => {
    const trace = optl.trace === undefined ? true : optl.trace;
    const metrics = optl.metrics === undefined ? true : optl.metrics;
    if (trace) {
      monitor.addTraceExporter(
  ...
  if (config.store) {
    const trace = config.store.trace === undefined ? true : config.store.trace;
    const metrics =
      config.store.metrics === undefined ? true : config.store.metrics;
    if (trace || metrics) {
  ...
  if (config.console) {
    const trace =
      config.console.trace === undefined ? true : config.console.trace;
    const metrics =
      config.console.metrics === undefined ? true : config.console.metrics;
    if (trace) {
