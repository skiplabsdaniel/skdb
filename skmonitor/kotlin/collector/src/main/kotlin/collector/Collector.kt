...
fun main(args: Array<String>) {
  ...
  if (engine == null) {
    logger.log(Level.SEVERE, "kts script engine not found")
    return
  }
  ...
  val filePath = "/tmp/skmonitor.trace"
  val file = File(filePath)
  if (file.exists()) {
    exec("mv", filePath, filePath + "." + System.currentTimeMillis())
  }
  exec("mkfifo", filePath)
  val managers = ArrayList<SpanManager>()
  managers.add(OpenTelSpanManager(otel.processor, spanLimits, clock))
  computeMetrics(meter, metrics, spanLimits, managers, engine)
  val pipe = RandomAccessFile(filePath, "r")
  try {
    var line: String?
    var stop = false
    while (!stop) {
      ...
      if (line == "exit") {
        ...
        try {
          val jsonSpan: JsonNode = jsonMapper.readValue(line, JsonNode::class.java)
          val skSpan = spanFromJSON(jsonSpan)
          for (manager in managers) {
            manager.manage(skSpan)
          }
