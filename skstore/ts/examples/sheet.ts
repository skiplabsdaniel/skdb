...
class SpreadSheets implements ReactiveStorage {
  ...
  initReactiveFlow = (
    ...
    const skall = cells.map((row, _occ) => {
    ...
    const evaluator = store.lazy<[string, string], string>((selfHdl, key) => {
      // TODO: Fix direct use of proxy
      const sheet = key[0];
      const cell = key[1];
      const temp: [string, string] = [sheet, cell];
    ...
    const skcomputed = skall.map((key, it) => {
      // TODO: writer.set fails with JSON imported object
      const sheet = key[0];
      const cell = key[1];
      const v = it.uniqueValue();
      if (typeof v == "string" && v.charAt(0) == "=") {
        return Array([[sheet, cell], evaluator.get([sheet, cell])]);
      }
      if (v == null) {
        throw new Error("(sheet, cell) pair must be unique.");
      }
      return Array([[sheet, cell], v]);
    });
    // Back to SKDB world
