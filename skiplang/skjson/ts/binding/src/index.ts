...
import { Type, type Binding, type CJCustom } from "./binding.js";
import type { Pointer, Nullable } from "../skiplang-std/index.js";
export type { Pointer, Nullable, Binding, CJCustom };
export { Type };
...
export const reactiveObject = {
  get<T, Base extends { [k: string]: Exportable<T> }>(
    hdl: ObjectHandle<T, Internal.CJObject>,
    prop: string | symbol,
  ...
  set<T>(
    _hdl: ObjectHandle<T, Internal.CJObject>,
    _prop: string | symbol,
    _value: any,
  ) {
  ...
  ownKeys<T>(hdl: ObjectHandle<T, Internal.CJObject>) {
...
function interpretPointer<T1, T extends Internal.CJSON>(
  ...
  switch (cjtype) {
    ...
    case Type.Object: {
      ...
      return new Proxy(
        new ObjectHandle<T, Internal.CJObject>(binding, oPtr),
        reactiveObject,
...
class ObjectHandle<T1, T extends Internal.CJSON> {
...
export class JsonConverterImpl<T> implements JsonConverter<T> {
  constructor(
    ...
    private conv?: CustomConverter<T>,
  ) {}
  ...
  public is(v: Pointer<Internal.CJSON>, type: Type | CJCustom): boolean {
